...
class ClientAddressCreateV2(BaseModel):
...
class ClientExistingCompanyLinkInputV2(BaseModel):
...
class ClientNewCompanyInputV2(BaseModel):
    ...
    address: ClientAddressCreateV2 | None = Field(
        None,
        description="Adresse à créer en même temps que la société",
    )


class ClientAccountCreateV2(BaseModel):
    ...
    language_id: int = Field(..., description="ID langue (table languages)")
    ...
    existing_company: ClientExistingCompanyLinkInputV2 | None = Field(
        None,
        description="Chemin 1: rattachement à une company déjà existante",
    )
    new_company: ClientNewCompanyInputV2 | None = Field(
        None,
        description="Chemin 2: création d'une nouvelle company",
    )


class ClientAccountCreatedV2(BaseModel):
    user_id: int
    role_id: int | None

...
from app.schemas.suppliers_v2 import (
    SupplierAccountCreateV2,
    SupplierAddressCreateV2,
    SupplierNewCompanyInputV2,
)


ROLE_NAME_SUPPLIER = "Fournisseur"
...
def get_supplier_role_id(db: Session) -> int | None:
    return get_role_id_by_name(db=db, role_name=ROLE_NAME_SUPPLIER)
...
def create_user(db: Session, payload: SupplierAccountCreateV2, role_id: int) -> User:
    user = User(
...
def create_user_profile(db: Session, user_id: int, payload: SupplierAccountCreateV2) -> None:
    profile = UserProfile(
        user_id=user_id,
...
def create_company(db: Session, payload: SupplierNewCompanyInputV2) -> Company:
    company = Company(
...
def create_company_address(
    ...
    address: SupplierAddressCreateV2,
) -> None:
    db_address = Address(
